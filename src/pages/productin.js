...
class productIn extends Component {
  ...
  componentDidMount = () =>  {
    ...
    axios
    .get(`https://191d92824391.ngrok.io/centralkitchen/getPOData`)
    .then(result => {
      this.setState({
        ...this.state,
  ...
  modalAddOpen = async () =>  {
    ...
    await axios
    .get(`https://191d92824391.ngrok.io/centralkitchen/getFormPOData`)
    .then( result => {
      this.setState({
        ...this.state,
  ...
  submitData = () => {
    ...
    if(dataToSend.VENDORCODE === ""){
      ...
      axios
      .post(`https://191d92824391.ngrok.io/centralkitchen/addFormPOData`, dataToSend, {
        headers: {
          "Access-Control-Allow-Origin": "*"
        }
      })
      .then(async result => {
        ...
        window.open(`http://localhost:3000/purchaseOrderPrint?ID=${result.data.id_po_h}`, "_blank")
